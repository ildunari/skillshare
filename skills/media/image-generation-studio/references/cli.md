...
export IMAGE_GEN="$CODEX_HOME/skills/image-generation-studio/scripts/image_gen.py"
```
...
```
mkdir -p tmp/image-generation-studio
cat > tmp/image-generation-studio/prompts.jsonl << 'EOF'
...
python "$IMAGE_GEN" generate-batch --input tmp/image-generation-studio/prompts.jsonl --out-dir out --concurrency 5

# Cleanup (recommended)
rm -f tmp/image-generation-studio/prompts.jsonl
```
