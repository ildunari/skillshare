...
If you need a broad font survey, download the Google Fonts metadata with your normal browser or a reviewed one-off command, save it as a local JSON file, then run a small local filter script against that file. Do not pipe network responses directly into executable code.

Example local filter script:
