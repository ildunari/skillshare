...
description: Use when the task is interface design for product UI such as dashboards, admin panels, apps, tools, settings pages, or other interactive product surfaces. Do not use for landing pages, marketing sites, campaigns, or other marketing design work.
---
...
These actions are part of this skill. Use them automatically when the context calls for it — no slash command needed.

## Extract Patterns from Code

When the user asks to extract, pull, or reverse-engineer a design system from existing code:
...
When the user asks to audit or check code against the design system:

**Requires** `.interface-design/system.md` — if missing, suggest extract or build first.

1. Parse system rules from system.md
2. Read target UI files
3. Check for: spacing violations (off-grid values), depth violations (wrong strategy), color violations (off-palette), pattern drift (components not matching defined patterns)
4. Report violations with file:line, expected vs actual, and suggestions

## Critique a Build

When the user asks to critique, review craft, or improve what was just built:
...
When the user asks for design system status:

Read `.interface-design/system.md` and display: direction, foundation, depth strategy, tokens, patterns, last updated. If no system.md exists, suggest building or extracting.
