...
- `html2pptx` is a **draft-quality fallback path**, not the ship-ready default renderer.
- Use it for rapid prototyping, emergency recovery, or partial conversion when the IR renderer cannot be used.
- Before shipping, run the normal pipeline QA path (preflight + visual QA where applicable) and treat html2pptx output as needing manual review.
