---
name: serve-sim_EvanBacon
description: Use for viewing or driving a running Apple Simulator with the serve-sim CLI: stream iOS, iPad, or Apple Watch simulators into a preview, tap or gesture with normalized coordinates, send hardware buttons, rotate, inspect accessibility, manage simulator permissions, inject camera feeds, or toggle CoreAnimation debug overlays. Trigger on requests mentioning serve-sim, iOS simulator preview/control, simulator taps/gestures, camera injection, simulator permissions, or visual simulator automation. Do not use for Android, real iOS hardware, app builds/installs, booting simulators from scratch, or in-app React Native debugging.
license: Apache-2.0
---
# serve-sim
