...
## Lightweight Alternative: defuddle

For a **single static page** (article, blog post, docs page) where you only need clean markdown and don't need JS rendering, search, or bulk operations, use `defuddle` — it's free (no API credits), fast, and removes clutter:

```bash
defuddle parse <url> --md
defuddle parse <url> --md -o content.md   # save to file
defuddle parse <url> -p title             # extract metadata
```

Install if needed: `npm install -g defuddle`

**Use firecrawl when:** you need search, JS rendering, bulk scraping, crawling, link extraction, or interaction.
**Use defuddle when:** you just need clean markdown from one known static URL.

| Flag | defuddle output format |
|------|------------------------|
...
For temporary one-time scripts (batch scraping, data processing), use `.firecrawl/scratchpad/`. Organize into subdirectories when it makes sense:
...
| `--sources <list>` | `web`, `images`, `news` (default: web) |
| `--categories <list>` | `github`, `research`, `pdf` |
| `--tbs <value>` | Time filter: `qdr:h/d/w/m/y` |
| `--location <loc>` | Geo-targeting |
| `--country <code>` | ISO country code (default: US) |
| `--scrape` | Scrape full content of results |
| `--scrape-formats <f>` | Formats when `--scrape` enabled |
...
| Option | Description |
|--------|-------------|
| `-f, --format <formats>` | `markdown`, `html`, `rawHtml`, `links`, `screenshot`, `json` |
