...
## Overview
This is the top-level coordinator. It decides which specialist skills or agents to invoke, in what order, with what scope, and merges results into one coherent final answer.
...
- Load `subagent-orchestrator` for concrete spawn/wait orchestration.
- Load `planning-protocol` for staged execution discipline when the work needs a shared implementation plan.
- Load specialists only as needed (`web-research`, `correctness-bug-hunter`, `ui-ux-myth`, and other focused workers).
