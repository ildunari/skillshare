...
Note: `xurl --version` is not valid in xurl 1.1.0; use the `xurl version` subcommand.
...
- App/profile: `kosta-x`
- Account: `ildunari`
- User id: `1661162529916059648`
- OAuth2 is configured and `kosta-x` is the default xurl app.
- Developer app is on X pay-per-use with OAuth user authentication enabled, app type `Web App, Automated App or Bot`, callback `http://localhost:8080/callback`, and read/write permissions.

Use these checks, which do not expose secrets:
```bash
xurl auth status
xurl whoami
xurl search 'from:ildunari' -n 1
```

Safe default behavior on this machine:
- Read/search/list operations can be run directly when useful.
- Ask Kosta before writes: post, reply, quote, delete, like, repost, bookmark, follow/unfollow, block/mute, DM, media upload/posting, or app/credential changes.
- Do not read `~/.xurl`; it contains OAuth tokens and app credentials.
