...
For example, how you reflect the "hover" state of a button could be left to your judgment when the user is exploring a quick scaffold with best practices, but it could also be defined in the codebase and need an exact match. That codebase definition could refer to design tokens that do not yet exist in code and that change dark and light mode values. In this second example you need variables work just to add a hover state with proper dark and light mode support, while in the first scenario a simpler best-practice scaffold may be enough. This is the line you will be walking, and good judgment here means choosing the most appropriate level of rigor for the environment.
