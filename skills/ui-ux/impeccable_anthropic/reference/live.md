...
```bash
node <this skill directory>/scripts/live.mjs
```
...
```
LOOP:
  node <this skill directory>/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node <this skill directory>/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div"
```
...
```bash
node <this skill directory>/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node <this skill directory>/scripts/live-server.mjs stop
```
...
```bash
node <this skill directory>/scripts/detect-csp.mjs
```
