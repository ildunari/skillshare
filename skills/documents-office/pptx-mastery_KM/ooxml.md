...
## Animations (builds) via `<p:timing>`

PowerPoint stores shape animations per slide under the `<p:timing>` element in `ppt/slides/slideN.xml`.

### Key takeaways

- Animations are **not** a single property on a shape; they are a **timeline tree**.
- The most common production pattern is a **click-advanced main sequence** where each click triggers the next build step.
- `PptxGenJS` does not provide stable low-level control of this OOXML tree, so pptx-master injects it in a post-pass:
  - see `scripts/inject_animations.py`
  - and the design/OOXML notes in `references/animations.md`

### Minimal click-advanced timing skeleton
...
The most robust entrance behaviors are:

- `<p:animEffect transition="in" filter="fade">` for fades/appears
- `<p:animEffect transition="in" filter="wipe(right|left|up|down)">` for directional reveals
- `<p:animScale>` for zoom (usually combined with a 1ms fade-in so the object starts hidden)

**Important:** PowerPoint is strict; keep to known-good patterns and validate by round-tripping through LibreOffice (`soffice --convert-to pdf`) after edits.

### References

- Open XML SDK — animation walkthrough: https://learn.microsoft.com/en-us/office/open-xml/presentation/working-with-animation-in-presentations
- OOXML timing examples: https://ooxml.info/docs/pptx/animations/
- Office implementation notes (MS-OE376): https://learn.microsoft.com/en-us/openspecs/office_standards/ms-oe376/
