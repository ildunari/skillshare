...
- Upstream-demo voice profiles (2026-06-10): upstream's `run_misotts.py` demo uses NO prompt audio — it's the model's base voice with each utterance chained on prior generated segments (that's where its consistency comes from). Kosta rated that demo 8/10, so both demo speakers were extracted into profiles: `upstream-demo-a` (speaker 0, 8.2s, anchors well — gate-passing takes at 0.99) and `upstream-demo-b` (speaker 1, only 2.8s — TOO SHORT to anchor reliably, never cleared the 0.975 gate in 3 tries; keep references ≥8s when building profiles). Local gate refs in `~/.hermes/voice-refs/`. A generated take that Kosta likes can itself be bottled as a profile — model-generated audio is perfectly in-distribution reference material, often better than movie-sourced clips.
