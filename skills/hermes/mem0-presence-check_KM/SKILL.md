...
- On the Mac Studio, mem0_oss routing is split across runtime plugin code and config: `~/.hermes/plugins/memory/mem0_oss/__init__.py`, `~/.hermes/mem0-oss.json`, and the GPT symlinked config. Do not assume Qwopus is the active primary; inspect the live mem0 config first. If Qwopus is used, it should point at Atomic Qwopus (`custom:atomic-qwopus` / `qwopus-atomic` / `http://100.93.10.54:1338/v1`) rather than stale `custom:RTX` / `qwopus-gpu` / `8010`. After plugin code edits, restart/new-process the gateway before expecting Telegram/Discord to pick them up.
