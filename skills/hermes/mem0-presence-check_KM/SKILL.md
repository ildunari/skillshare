...
- On the Mac Studio, mem0_oss Qwopus/Spark routing is split across runtime plugin code and config: `~/.hermes/plugins/memory/mem0_oss/__init__.py`, `~/.hermes/mem0-oss.json`, and the GPT symlinked config. Qwopus is the primary extraction LLM, Spark is the fallback, and smart routing uses `~/.hermes/shared/qwopus-router.sqlite` leases to avoid dogpiling Qwopus across separate gateway/cron/CLI processes. After plugin code edits, restart/new-process the gateway before expecting Telegram/Discord to pick them up.
