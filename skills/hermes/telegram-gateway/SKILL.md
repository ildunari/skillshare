...
- Telegram native draft streaming via Bot API `sendMessageDraft` (`streaming.transport: draft` / `draft_mode: true`), committed as `dd118b561` on `local/studio-customizations`; it applies only to Telegram DMs and falls back to edit streaming if drafts fail
...
- Telegram native draft streaming is currently disabled for DM topics. Normal/final Hermes sends must keep using `message_thread_id` for visible DM-topic placement; draft endpoints had different/unstable behavior and produced duplicates, progress in `All`, and disappearing topic replies. Keep `streaming.transport: edit` / `draft_mode: false` for Telegram DM topics until the draft lifecycle has an end-to-end proof. See `references/telegram-dm-topic-draft-routing-2026-05-08.md` and `references/telegram-dm-topic-draft-rollback-2026-05-08.md`.
