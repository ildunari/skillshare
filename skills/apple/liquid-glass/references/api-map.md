...
| `Glass.identity` | Disable/neutral off-state in glass transitions | Use for state changes where the element should temporarily stop being glass. |
...
| `GlassEffectContainer(spacing:)` | Related controls that should blend/morph as one group | Use around composer controls, FAB clusters, toolbar groups, and segmented controls. |
| `.glassEffectID(_:in:)` | Morphing a glass element between compact/expanded states | IDs should be stable and unique within the namespace. |
